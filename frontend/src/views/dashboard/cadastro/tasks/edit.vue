...
<script>
  ...
  export default {
    ...
    data () {
      return {
        dialogAtualizar: false, valid: true, camposForm: null, url: null, 
    ...
    methods: {
      ...
      listDados() {
        this.resetVariaveis()
        this.dialog = true
        this.$store.dispatch('loadDados', this.$enderecoAPI + '/tasks?id='+this.$route.params.id)
        .then(response => {
          this.descricao_model = response.data.data[0].descricao;
          this.observacao_model = response.data.data[0].observacao;
          this.status_model = response.data.data[0].id_status;
        }).catch(error => {
      ...
      salvar() {        
        if (this.$refs.form.validate()) {        
          this.url = this.$enderecoAPI + '/tasks/'+this.$route.params.id
